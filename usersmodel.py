...
class UsersModel():
    ...
    def insert(self, username, name, surname, password_hash, rank):
        cursor = self.connection.cursor()
        status = ''
        photo_url = 'https://gramps-project.org/wiki/images/2/22/Macos_200x200.png'
        cursor.execute('''INSERT INTO users 
                          (username, name, surname, status, password_hash, photo_url, rank) 
                          VALUES (?,?,?,?,?,?,?)''', (username, name, surname, status, password_hash, photo_url, rank))
        cursor.close()
        self.connection.commit()     
